...
{
  /// <summary>
  /// Страница с закрытыми задачами.
  /// </summary>
  internal class TasksAtClose : IBotMessage
  {
    /// <summary>
    ...
    public async Task InLineKeyBoardProcessingAsync(Update update, ITelegramBotClient botClient, CancellationToken token)
    {
      ...
      {
        case "keyViewCloseTask":
          StatusMenu.PlaceMenu = Place.ViewCloseTask;
        ...
        case "keyCloseMenu":
          ...
          break;
        case "keyCancel":
          await botClient.SendTextMessageAsync(chatID, $"Действие отменено", replyMarkup: buttonsMap.GetTasksAtCloseMenu(), cancellationToken: token);
          break;
      }
    }
  }
